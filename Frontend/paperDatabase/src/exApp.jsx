...
export default function App() {
  ...
  function handleGetUser1() {
    ...
    const auth = Buffer.from(authentication.username + ':' + authentication.password).toString('base64');
    headers.set('Authorization', 'Basic ' + auth);
    return fetch('http://localhost:8083/users/test', { method: 'GET', headers: headers })
      .then(response => response.text())
      .then(text => setMessage(text))
      .catch(error => console.log('ERROR: ' + error));
  }
  ...
  return (
    <>
      ...
      <button type='button' onClick={handleGetUser1}>
        get user1
      </button>
      <button type='button' onClick={handleAuthorizedUser}>
        authorized user
      </button>
      <p>{message}</p>
    </>
